...
cmd_fix() {
  ...
  # Python packages
  if command -v python3 &>/dev/null; then
    python3 -m pip install --break-system-packages -q reportlab pypdf matplotlib playwright 2>/dev/null \
      || python3 -m pip install -q reportlab pypdf matplotlib playwright 2>/dev/null \
      || { yellow "  pip install failed — try: pip install reportlab pypdf matplotlib playwright"; rc=3; }
    green "  ✓ Python packages installed (reportlab, pypdf, matplotlib, playwright)"
  fi
