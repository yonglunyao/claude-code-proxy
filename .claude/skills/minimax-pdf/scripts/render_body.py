...
def _find_font_file(font_name: str, font_dirs: list) -> str | None:
    """Search for font file in given directories."""
    # 常见字体文件名映射
    name_variants = [
        font_name,
        font_name.replace(" ", ""),
    ...
    # 鸿蒙黑体特殊处理：HarmonyHeiTi -> Harmony-Regular/Harmony-Bold
    if "HarmonyHeiTi" in font_name:
        # 提取字重：HarmonyHeiTi -> Regular, HarmonyHeiTi-Bold -> Bold
        if "Bold" in font_name or "bold" in font_name:
            name_variants.extend(["Harmony-Bold", "HarmonyBold", "Harmony_Bold"])
        else:
            # 常规体
            name_variants.extend(["Harmony-Regular", "HarmonyRegular", "Harmony_Regular", "Harmony"])

    for font_dir in font_dirs:
        for root, dirs, files in os.walk(font_dir):
            for filename in files:
                name_without_ext = os.path.splitext(filename)[0]
                if name_without_ext in name_variants:
...
def register_fonts(tokens: dict):
    ...
    # 检查是否需要加载 HarmonyHeiTi 字体
    display_font = tokens.get("font_display_rl", "")
    body_font = tokens.get("font_body_rl", "")

    # 如果需要中文字体，直接使用已知路径注册
    if "HarmonyHeiTi" in display_font or "HarmonyHeiTi" in body_font:
        registered = pdfmetrics.getRegisteredFontNames()

        # 鸿蒙黑体字体文件路径（已知位置）
        harmony_fonts = {
            "HarmonyHeiTi": "/usr/share/fonts/HarmonyFont/Harmony-Regular.ttf",
            "HarmonyHeiTi-Bold": "/usr/share/fonts/HarmonyFont/Harmony-Bold.ttf",
        }

        for font_name, font_path in harmony_fonts.items():
            if font_name not in registered:
                if os.path.exists(font_path):
                    try:
                        pdfmetrics.registerFont(TTFont(font_name, font_path))
                    except Exception as e:
                        print(f"Warning: Failed to register {font_name}: {e}", file=sys.stderr)
                else:
                    # 如果已知路径不存在，尝试搜索
                    font_dirs = _get_system_font_paths()
                    # 搜索 Harmony-Regular 或 Harmony-Bold 文件
                    search_names = ["Harmony-Regular", "Harmony-Bold"] if "Bold" in font_name else ["Harmony-Regular"]
                    for search_name in search_names:
                        found_path = _find_font_file(search_name, font_dirs)
                        if found_path:
                            try:
...
def get_fallback_fonts(tokens: dict) -> dict:
    """Return tokens with fallback fonts if requested fonts aren't registered."""
    registered = pdfmetrics.getRegisteredFontNames()
    t = tokens.copy()

    # Define fallbacks
    fallbacks = {
        t.get("font_display_rl", ""): "Times-Bold",
        t.get("font_body_rl", ""): "Helvetica",
        t.get("font_body_b_rl", ""): "Helvetica-Bold",
    }

    # Apply fallbacks for unregistered fonts
